    ...
    public boolean insertHerramientas(Herramientas h ){
        try {
          String sql ="insert into herramientas values("+0+",'"+h.getNombre_herr()+"','"+h.getDescripcion_herr()+"',"+h.getValor_herr()+",'"+h.getMarca_herr()
                  +"','"+h.getFecha_ing_herr()+"','"+h.getFecha_mod_herr()+"',(select id_prov from proveedor where nombre_prov='"+h.getNombre_prov()+"'),active)";
...
Herramientas herr= new Herramientas();
        ...
        try {
            PreparedStatement pstm = this.getConexion().prepareStatement("SELECT h.id_herr , h.nombre_herr, h.descripcion_herr,h.valor_herr,h.fecha_ing_herr, h.fecha_mod_herr,p.id_prov,p.nombre_prov, h.state_herr \n"
